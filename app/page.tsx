...
const labels = {
  en: { subtitle: 'Food Safety Practice Test', home: 'Home', title: 'Balady Food Safety Official Practice Exam', desc: 'Complete self-assessment exam covering municipal food safety, HACCP management, personal hygiene, and hazard control regulations.', total: 'Total Questions', time: 'Time Allocation', minute: '90 Sec / Q', pass: 'Passing Mark', start: 'Start Timed Practice Exam', question: 'Question', timer: 'Timer:', previous: 'Previous', next: 'Next', submit: 'Submit Exam', loading: 'Loading questions from Firebase...', empty: 'No questions have been added yet.', passed: 'Congratulations! You Passed', failed: 'Test Not Passed', resultSubtitle: 'Minimum passing threshold is 80%. Review your incorrect answers and retry.', correct: 'Correct', wrong: 'Wrong', skipped: 'Skipped', score: 'Final Score Percentage', benchmark: 'Official Municipal Benchmark is 80%', review: 'Review Answers', retry: 'Retry Test', reviewTitle: 'Answer Review', yourAnswer: 'Your answer', correctAnswer: 'Correct answer', unanswered: 'Not answered', backResult: 'Back to Result' },
  bn: { subtitle: 'খাদ্য নিরাপত্তা অনুশীলন পরীক্ষা', home: 'হোম', title: 'বলদিয়া খাদ্য নিরাপত্তা অফিসিয়াল অনুশীলন পরীক্ষা', desc: 'পৌর খাদ্য নিরাপত্তা, HACCP ব্যবস্থাপনা, ব্যক্তিগত পরিচ্ছন্নতা এবং ঝুঁকি নিয়ন্ত্রণের সম্পূর্ণ মূল্যায়ন।', total: 'মোট প্রশ্ন', time: 'সময় বরাদ্দ', minute: '৯০ সেকেন্ড / প্রশ্ন', pass: 'উত্তীর্ণের নম্বর', start: 'সময়ভিত্তিক অনুশীলন পরীক্ষা শুরু করুন', question: 'প্রশ্ন', timer: 'অবশিষ্ট সময়:', previous: 'পূর্ববর্তী', next: 'পরবর্তী', submit: 'পরীক্ষা জমা দিন', loading: 'প্রশ্ন লোড হচ্ছে...', empty: 'এখনও কোনো প্রশ্ন যোগ করা হয়নি।', passed: 'অভিনন্দন! আপনি উত্তীর্ণ হয়েছেন', failed: 'পরীক্ষায় উত্তীর্ণ হননি', resultSubtitle: 'উত্তীর্ণ হওয়ার ন্যূনতম সীমা ৮০%। ভুল উত্তরগুলো পর্যালোচনা করে আবার চেষ্টা করুন।', correct: 'সঠিক', wrong: 'ভুল', skipped: 'এড়িয়ে যাওয়া', score: 'চূড়ান্ত স্কোর শতাংশ', benchmark: 'সরকারি পৌরসভার মানদণ্ড ৮০%', review: 'উত্তর পর্যালোচনা', retry: 'আবার পরীক্ষা দিন', reviewTitle: 'উত্তর পর্যালোচনা', yourAnswer: 'আপনার উত্তর', correctAnswer: 'সঠিক উত্তর', unanswered: 'উত্তর দেওয়া হয়নি', backResult: 'ফলাফলে ফিরে যান' },
  ar: { subtitle: 'اختبار تدريبي لسلامة الأغذية', home: 'الرئيسية', title: 'اختبار ممارسة سلامة الأغذية الرسمي للبلدية', desc: 'اختبار شامل يغطي سلامة الأغذية البلدية ونظام الهاسب والنظافة الشخصية ومراقبة المخاطر.', total: 'إجمالي الأسئلة', time: 'الوقت المخصص', minute: '90 ثانية / سؤال', pass: 'درجة النجاح', start: 'بدء الاختبار التدريبي المؤقت', question: 'السؤال', timer: 'الوقت المتبقي:', previous: 'السابق', next: 'التالي', submit: 'إرسال الاختبار', loading: 'جار تحميل الأسئلة من Firebase...', empty: 'لم تتم إضافة أسئلة بعد.', passed: 'تهانينا! لقد نجحت', failed: 'لم تنجح في الاختبار', resultSubtitle: 'الحد الأدنى للنجاح هو 80٪. راجع إجاباتك غير الصحيحة وحاول مرة أخرى.', correct: 'صحيح', wrong: 'خطأ', skipped: 'متروك', score: 'النسبة النهائية', benchmark: 'المعيار الرسمي للبلدية هو 80٪', review: 'مراجعة الإجابات', retry: 'إعادة الاختبار', reviewTitle: 'مراجعة الإجابات', yourAnswer: 'إجابتك', correctAnswer: 'الإجابة الصحيحة', unanswered: 'لم تتم الإجابة', backResult: 'العودة إلى النتيجة' },
};
...
export default function HomePage() {
  ...
  useEffect(() => {
    listQuestions().then((items) => { setQuestions(items); setAnswers(new Array(items.length).fill(null)); setSeconds(items.length * SECONDS_PER_QUESTION); }).catch(() => setError('Unable to load questions. Check Firebase configuration.')).finally(() => setLoading(false));
  }, []);
  ...
  function startQuiz() { setAnswers(new Array(questions.length).fill(null)); setCurrent(0); setSeconds(questions.length * SECONDS_PER_QUESTION); setScreen('quiz'); }
  ...
  return <Shell language={language} setLanguage={switchLanguage} t={t}>
    {screen === 'start' && <div className="panel start-panel"><div className="icon-box">✓</div><h2>{t.title}</h2><p className="subtitle">{t.desc}</p><div className="stats"><div className="stat"><small>{t.total}</small><strong>{questions.length}</strong></div><div className="stat"><small>{t.time}</small><strong>{t.minute}</strong></div><div className="stat"><small>{t.pass}</small><strong>{PASS_PERCENTAGE}%</strong></div></div><button className="primary" onClick={startQuiz}>{t.start} →</button></div>}
